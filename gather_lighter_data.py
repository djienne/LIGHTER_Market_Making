...
from logging_config import setup_logging as _setup_logging, setup_summary_logger, setup_performance_logger
logger = _setup_logging('gather_lighter_data', log_filename='debug.log')
summary_logger = setup_summary_logger()
setup_performance_logger()  # registers 'performance' logger; accessed via logging.getLogger('performance')



# --- Configuration ---
CONFIG_FILE = 'config.json'
_DEFAULT_TICKERS = ['ETH', 'BTC', 'SOL', 'PAXG']

def _load_full_config():
    """Loads the full configuration dict from config.json."""
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred while loading '{CONFIG_FILE}': {e}. Using defaults.")
        return {}

def load_config():
    """Loads CRYPTO_TICKERS from config.json."""
...
BUFFER_SECONDS = int(os.getenv(
    "BUFFER_SECONDS",
    _full_config.get("performance", {}).get("buffer_seconds", 5)))
    ...
    for symbol in CRYPTO_TICKERS:
        ...
        if file_path:
            try:
                ...
                df = pd.read_parquet(file_path, engine='pyarrow', columns=['trade_id'])
