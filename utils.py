    ...
    try:
        ...
        for order_book in order_books_response.order_books:
            if order_book.symbol.upper() == symbol.upper():
                ...
                if not (0 <= order_book.supported_price_decimals <= 18):
                    raise ValueError(f"Anomalous price decimals ({order_book.supported_price_decimals}) for {symbol}")
                if not (0 <= order_book.supported_size_decimals <= 18):
                    raise ValueError(f"Anomalous size decimals ({order_book.supported_size_decimals}) for {symbol}")
...
def prepare_calculation_windows(mid_price_df: pd.DataFrame, trades_df: pd.DataFrame,
                                ...
                                periods_to_keep: int = 8) -> Tuple[pd.DataFrame, pd.DataFrame]:
