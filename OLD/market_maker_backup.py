...
if os.path.exists('market_maker_debug.txt'):
    os.remove('market_maker_debug.txt')

file_handler = logging.FileHandler('market_maker_debug.txt', mode='w')
...
def load_avellaneda_parameters():
    ...
    try:
        ...
        if avellaneda_params is None or (current_time - last_avellaneda_update) > AVELLANEDA_REFRESH_INTERVAL:
            with open('avellaneda_parameters_PAXG.json', 'r') as f:
        ...
        logger.warning("⚠️ avellaneda_parameters_PAXG.json not found, using static spread")
        return False
    except Exception as e:
...
async def market_making_loop(client, api_client):
    ...
    while True:
        ...
        # This part of the original code is unreachable in a `while True` loop without a break,
        # but kept for graceful shutdown if a break is added later.
