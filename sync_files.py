...
def build_pre_ssh_command(host, remote_path, identity_file=None):
    ...
    remote_sequence = (
        ...
        f"sudo chown -R $(whoami):$(whoami) {shlex.quote(remote_path)}"
    )
    cmd = ["ssh"]
    if identity_file:
        cmd.extend(["-i", identity_file])
...
def main():
    parser = argparse.ArgumentParser(description="Sync lighter_MM component files to a remote host")
    parser.add_argument(
        "--host",
    ...
    # PRE-FLIGHT SSH: Ensure remote directory exists and has correct ownership
